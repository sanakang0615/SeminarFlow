...
const data = [
  ...
  { id: 5, action: "컨트롤 그룹이 트리트먼트 그룹과 동일한 질문을 올렸을 때, 폴리의 답변과 기존 서비스의 답변은 얼마나 차이가 나나요?", comment: "제가 알기로는, 이것이 정말 훌륭한 기능이라고 생각합니다. 검색 기능이 레그에 그대로 적용되었고, 동일한 검색 데이터가 프롬프트에 반영되기 때문에 GPT가 같은 내용을 바탕으로 솔루션을 만들어내고 있습니다.",date: "32:26" },
        ...
        {/* <div className="max-w-lg mx-auto my-10"> */}
          {data.map((item, index) => (
            <div key={item.id} className="relative mb-4">
              {/* Vertical line, conditionally rendered if not the last item */}
              {index < data.length - 1 && (
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1 ml-4 mt-1">
                  {/* Date shown before the action text */}
